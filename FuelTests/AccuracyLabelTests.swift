...
struct AccuracyLabelTests {
    ...
    func drawnStyle() {
        let style = FuelTypography.overlayCaption
    ...
    func doesNotTransform() {
        #expect(FuelTypography.overlayCaption.isUppercased == false)
    ...
    func pinnedAgainstDynamicType() {
        #expect(FuelTypography.overlayCaption.scalesRelativeTo == nil)
    }
    ...
    func smallerThanTheNumber() {
        #expect(FuelTypography.overlayCaption.size < FuelTypography.listValue.size)
        #expect(FuelTypography.overlayCaption.size < FuelTypography.resultCalories.size)
    }
